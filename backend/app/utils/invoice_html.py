...
def build_invoice_html(
  *,
  ...
  kwargs_common = dict(
    ...
    footer_text=footer, banking_html=banking_html, primary_color=prim, secondary_color=sec,
  )
  if theme == "elegant":
...
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"/><title>{doc_label} #{doc_number}</title></head>
<body style="font-family:Georgia,serif;max-width:700px;margin:40px auto;padding:20px;color:#333">
  ...
  <table style="width:100%;margin-top:20px">
    <tr>
      <td style="width:50%;vertical-align:top">
        ...
        {f'<p style="margin:2px 0 0;color:#666;font-size:13px">{customer_email}</p>' if customer_email else ''}
      </td>
      <td style="text-align:right;vertical-align:top">
  ...
  <table style="width:100%;margin-top:16px;border-collapse:collapse">
    ...
    <tr style="font-weight:bold;font-size:1.1em"><td style="padding:12px 8px;text-align:right">Total</td><td style="text-align:right">{currency} {_fmt_num(total)}</td></tr>
  </table>
  ...
  <div style="padding:32px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:24px;margin-bottom:28px">
      <div>
        ...
        {f'<div style="color:#6b7280;font-size:13px">{customer_email}</div>' if customer_email else ''}
      </div>
      <div style="text-align:right">
    ...
    <div style="margin-top:24px;text-align:right;background:#f9fafb;padding:20px 24px;border-radius:8px">
      ...
      <div style="font-size:18px;font-weight:700;margin-top:12px;padding-top:12px;border-top:1px solid #e5e7eb">Total <span style="float:right">{currency} {_fmt_num(total)}</span></div>
    </div>
...
<body style="font-family:Helvetica,Arial,sans-serif;max-width:560px;margin:0 auto;padding:40px 24px;color:#000;font-size:14px">
  ...
  {f'<p style="color:#666;margin-bottom:20px;font-size:13px">{customer_email}</p>' if customer_email else ''}
  <table style="width:100%;margin:24px 0;border-collapse:collapse">
    <tbody>{rows}</tbody>
  </table>
  <p style="text-align:right;margin-top:16px;font-size:18px;font-weight:600">Total {currency} {_fmt_num(total)}</p>
...
<body style="font-family:'Cormorant Garamond',Georgia,serif;max-width:680px;margin:0 auto;padding:48px 32px;color:#2c2825;background:#faf9f7">
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:32px">
    <div>
      ...
      {f'<p style="margin:4px 0 0;font-size:14px;color:#6b6560">{customer_email}</p>' if customer_email else ''}
    </div>
    <div style="text-align:right">
  ...
  <table style="width:100%;border-collapse:collapse;font-size:14px">
    <thead><tr style="background:#f0ebe5;color:#5a534d;font-size:11px;text-transform:uppercase;letter-spacing:0.08em">
  ...
  <div style="margin-top:28px;text-align:right;padding:20px 24px;background:#f5f1eb;border-radius:4px">
    ...
    <p style="margin:12px 0 0;font-size:18px;font-weight:600">Total <span style="margin-left:16px">{currency} {_fmt_num(total)}</span></p>
  </div>
  ...
  <div style="padding:32px 40px">
    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:24px;margin-bottom:28px;padding:20px;background:#1f2937;border-radius:8px">
      <div>
        ...
        {f'<p style="margin:4px 0 0;color:#9ca3af;font-size:14px">{customer_email}</p>' if customer_email else ''}
      </div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px">
      <thead><tr style="background:#374151;color:{secondary_color};font-size:11px;text-transform:uppercase">
        <th style="padding:14px 20px;text-align:left">Description</th>
        <th style="padding:14px 20px;text-align:right">Qty</th>
        <th style="padding:14px 20px;text-align:right">Unit price</th>
        <th style="padding:14px 20px;text-align:right">Amount</th>
      </tr></thead>
      <tbody>{rows}</tbody>
    </table>
    <div style="margin-top:24px;text-align:right;padding:24px;background:#1f2937;border-radius:8px;border:1px solid #374151">
      ...
      <p style="margin:16px 0 0;font-size:20px;font-weight:700;color:{secondary_color}">Total <span style="float:right">{currency} {_fmt_num(total)}</span></p>
    </div>
  ...
  <div style="padding:36px;border:1px solid #e2e8f0;border-top:none">
    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:24px;margin-bottom:28px;padding-bottom:20px;border-bottom:1px solid #e2e8f0">
      <div>
        ...
        {f'<p style="margin:4px 0 0;color:#64748b;font-size:13px">{customer_email}</p>' if customer_email else ''}
      </div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px">
      <thead><tr style="background:#f1f5f9;color:#475569;font-size:11px;text-transform:uppercase;font-weight:600">
    ...
    <div style="margin-top:24px;text-align:right;background:#f8fafc;padding:20px 24px;border-radius:6px;border:1px solid #e2e8f0">
      ...
      <p style="margin:14px 0 0;font-size:18px;font-weight:700;color:{primary_color};padding-top:12px;border-top:1px solid #e2e8f0">Total <span style="float:right">{currency} {_fmt_num(total)}</span></p>
    </div>
